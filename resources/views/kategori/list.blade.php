                    ...
                    @if ($kategori->isNotEmpty())
                        @foreach ($kategori as $kate)
                            <tr class="border-b">
                                ...
                                <td class="px-6 py-3 text-left">
                                    {{ \Carbon\Carbon::parse($kate->created_at)->format('d M, Y') }}
                                </td>
                                <td class="px-6 py-3 text-center">
                                    <a href="{{ route('kategori.edit', $kate->id) }}"
                                        class="bg-slate-700 text-sm rounded-md text-white px-3 py-2 hover:bg-slate-800">Edit</a>
                                    <a href="javascript:void(0);" onclick="deleteKategori({{ $kate->id }})"
