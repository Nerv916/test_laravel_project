                    ...
                    @if ($satuan->isNotEmpty())
                        @foreach ($satuan as $stn)
                            <tr class="border-b">
                                ...
                                <td class="px-6 py-4">{{ $stn->name }}</td>
                                <td class="px-6 py-3 text-left">
                                    {{ \Carbon\Carbon::parse($stn->created_at)->format('d M, Y') }}
                                </td>
                                <td class="px-6 py-3 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a href="{{ route('satuan.edit', ['id' => $stn->id]) }}"
                                            class="bg-slate-700 text-sm rounded-md text-white px-3 py-2 hover:bg-slate-800">
                                            Edit
                                        </a>
                                        <a href="javascript:void(0);"
                                            onclick="event.preventDefault(); deleteSatuan({{ $stn->id }});"
